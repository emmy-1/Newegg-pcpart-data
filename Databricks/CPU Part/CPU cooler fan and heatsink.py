...
schema = StructType([
    ...
    StructField("Noise Level", StringType(), True),
...
for page in range(1, 5):
    if page == 1:
        Html_ = get_html(f'https://www.newegg.com/global/uk-en/tools/custom-pc-builder/pl/ID-574?diywishlist=0&isCompability=false')
    else:
        Html_ = get_html(f'https://www.newegg.com/global/uk-en/tools/custom-pc-builder/pl/ID-574/Page-{page}?diywishlist=0&isCompability=false')
    ...
    if table is not None:
        ...
        for td in td_elements:
            ...
            data.append((title, RPM , Noise_Level, Color,ratings, price_value, image_url, tips_value))

# Create the DataFrame from the collected data            
data_df = spark.createDataFrame(data, schema=schema)

# Write the DataFrame to a CSV file
data_df.coalesce(1).write.mode("overwrite").csv('abfss://pcpart@neweggdb.dfs.core.windows.net/Dataset/Raw/FanandHeatsink', header=True)
