...
schema = StructType([
    ...
    StructField("Wattage", StringType(), True),
    StructField("Modular_type", StringType(), True),
...
for page in range(1, 14):
    if page == 1:
        Html_ = get_html(f'https://www.newegg.com/global/uk-en/tools/custom-pc-builder/pl/ID-58?diywishlist=0&isCompability=false')
    else:
        Html_ = get_html(f'https://www.newegg.com/global/uk-en/tools/custom-pc-builder/pl/ID-58/Page-{page}?diywishlist=0&isCompability=false')
    ...
    if table is not None:
        ...
        for td in td_elements:
            ...
            if div is not None and span is not None:
                ...
                if label == 'Type':
                    Type = value
                elif label == 'Energy-Efficient':
                    Energy_Efficient = value    
                elif label == 'Wattage':
                    Wattage = value    
                elif label == 'Modular':
                    Modular_type = value
            ...
            data.append((title, Type, Energy_Efficient, Wattage, Modular_type, ratings, price_value, image_url, tips_value))
            
# Create the DataFrame from the collected data            
data_df = spark.createDataFrame(data, schema=schema)

# Write the DataFrame to a CSV file
data_df.coalesce(1).write.mode("overwrite").csv('abfss://pcpart@neweggdb.dfs.core.windows.net/Dataset/Raw/PowerSupply', header=True)
