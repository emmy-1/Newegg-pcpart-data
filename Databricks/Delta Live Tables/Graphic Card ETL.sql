...
AS SELECT Title, GPU, CAST(Memory AS INT), CAST(PSU AS INT), CAST(`Length` AS INT), CAST(Ratings AS INT), CAST(Price AS INT),`Image URL`,CAST(Tips AS INT)
FROM cloud_files('abfss://pcpart@neweggdb.dfs.core.windows.net/Dataset/DLT_Bronze_layer/', 'csv')
...
AS SELECT Title, GPU, Memory as MemorySize , PSU as RequiredPSUWattage,`Length` as ItemLength, Ratings as RatingCount,Price,`Image URL` as ImageURL,Tips as CustomersAdded
